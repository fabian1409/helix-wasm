...
// PATCHED for helix-wasm: `libloading::Library`/`Symbol` (dlopen) don't exist on
// wasm32-wasip1 (no `unix`/`windows` cfg matches there) - gate them out so this crate can
// compile on that target when only the `tree-sitter-language` static-linking path below is
// used. See vendor/tree-house-bindings/README.md in the helix-wasm repo for the full story.
#[cfg(not(target_arch = "wasm32"))]
