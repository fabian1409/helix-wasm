...
extern "C" {
    ...
    /// will be executed.
    // PATCHED for helix-wasm: the C function returns `bool` (vendor/include/tree_sitter/api.h)
    // but this declaration previously omitted it. Harmless on native ABIs where the extra
    // return value is just discarded, but wasm's strict `call` type-checking traps on
    // exactly this mismatch (`signature_mismatch:ts_query_cursor_set_byte_range`) - see
    // PATCH.md.
    fn ts_query_cursor_set_byte_range(self_: *mut QueryCursorData, start_byte: u32, end_byte: u32) -> bool;
