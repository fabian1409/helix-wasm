...
pub struct Nucleo<T: Sync + Send + 'static> {
    ...
    // No per-instance pool on wasm32 - see `Worker::new`'s wasm32 arm in worker.rs.
    #[cfg(not(target_arch = "wasm32"))]
    pool: ThreadPool,
    state: State,
    items: Arc<boxcar::Vec<T>>,
...
impl<T: Sync + Send + 'static> Nucleo<T> {
    ...
    fn tick_inner(&mut self, timeout: u64, canceled: bool, status: pattern::Status) -> Status {
        ...
        if running {
            ...
            // No pool to spawn onto on wasm32 (see `Worker::new`'s wasm32 arm in worker.rs) -
            // run inline instead of deferring.
            #[cfg(target_arch = "wasm32")]
            unsafe {
                inner.run(status, cleared)
            }
            #[cfg(not(target_arch = "wasm32"))]
            self.pool
